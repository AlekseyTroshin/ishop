...
class UserController extends AppController
{
    ...
    public function viewAction()
    {
        ...
        $this->setMeta("Админка :: {$title}");
        $this->set(compact('title', 'user', 'orders', 'pagination', 'total'));
    }
        ...
        if (!empty($_POST)) {
            ...
            if (!$this->model->validate($this->model->attributes)
                ...
                if ($this->model->update('user', $id)) {
                    $_SESSION['success'] = "Данные пользователя обновлены.<br>Перезайдите, если вы обновляли свои данные.";
                } else {
    ...
    public function addAction()
    {
        if (!empty($_POST)) {
            ...
            if (!$this->model->validate($this->model->attributes)
                || !$this->model->checkUnique('Этот E-mail уже занят')) {
                $this->model->getErrors();
                ...
                if ($this->model->save('user')) {
                    ...
                    $_SESSION['errors'] = 'Ошибка добавление пользователя';
                }
            }
